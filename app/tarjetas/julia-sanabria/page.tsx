...
export default function JuliaSanabria() {
  useEffect(() => {
  ...
  const downloadVCard = () => {
    const vcard = [
      ...
      'URL:https://nuvowin.com/tarjetas/julia-sanabria',
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-sm rounded-full shadow px-3 py-1 flex items-center gap-3 z-20">
            <button
              ...
              className="flex items-center gap-2  px-3 py-2 rounded-md text-xs text-gray-700 hover:bg-purple-50 transition"
            >
              <BsQrCode size={20} />
            </button>
            <button
            ...
            <button onClick={downloadVCard} className="flex items-center gap-2  px-3 py-2 rounded-md text-xs text-gray-700 hover:bg-red-50 transition">
              <FiDownload size={20} />
            </button>
          </div>
          ...
          {downloadMsg && (
            <div className="absolute top-14 left-1/2 -translate-x-1/2 z-30">
              <div className="bg-black text-white text-sm px-3 py-1 rounded">{downloadMsg}</div>
            </div>
          )}

          <div className="relative h-36">
            <div className="absolute inset-x-0 top-0 h-36 bg-gradient-to-r from-purple-600 to-purple-400" />
            <div className="absolute left-1/2 -bottom-14 -translate-x-1/2 w-[140px] h-[140px] rounded-full -z-0 flex items-center justify-center">
              <div className="w-full h-full rounded-full bg-gradient-to-tr from-purple-400 via-purple-300 to-transparent opacity-60 filter blur-2xl" />
            </div>
          ...
          <div className="pt-16 pb-6 px-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">Asesora Comercial</p>
            ...
            <div className="space-y-3 text-left">
              ...
              <a
                ...
                className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg border hover:bg-purple-50 transition"
              >
...
// QR Modal render at bottom of file (client-side) - simple implementation
function QRModal({
  open,
  onClose,
  qrUrl,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl p-6 w-[320px] flex flex-col items-center">
        <h3 className="text-lg font-semibold mb-4">
          QR - Tarjeta Julia Sanabria
        </h3>
        <img
          src={qrUrl}
          alt="QR Julia Sanabria"
        ...
        <div className="flex gap-3">
          <a
            ...
            href={qrUrl}
            download="julia-sanabria-qr.png"
          >
            Descargar
          </a>
...
function ShareModal({
  ...
  const onNativeShare = async () => {
    if ((navigator as any).share) {
      ...
      // fallback: copy and show transient message
      try {
  ...
  const onMail = (provider: "gmail" | "outlook") => {
    const subject = encodeURIComponent("Tarjeta de contacto - Julia Sanabria");
      ...
      <div className="bg-white rounded-xl p-4 w-[420px] max-w-[95%]">
        ...
        {msg && (
          <div className="mt-3 flex justify-center">
            <div className="bg-black text-white text-sm px-3 py-1 rounded">{msg}</div>
          </div>
        )}
      </div>
    </div>
